...
u1 = User.create(name:'lin', email:'lin@gmail.com', password:'1234')
u2 = User.create(name:'judy', email:'judy_g@gmail.com', password:'1234')

hamlet_roles = ['Hamlet', 'Ophelia', 'Polonius', 'Laertes', 'Horatio', 'Gertrude', 'Ghost' ]
hamlet_roles.each{|hr| CastMember.create(name:Faker::Name.name, role:hr, production:p1)}

cats_roles = ['Mr. Mistoffelees', 'Bombalurina', 'Rumpletezer', 'Grizabella']
cats_roles.each{|cr| CastMember.create(name:Faker::Name.name, role:cr, production:p2)}

carmen_roles = ['Carmen', 'Escamillo', 'Jose', 'Mercedes', 'Dancaire']
carmen_roles.each{|cr|CastMember.create(name:Faker::Name.name, role:cr, production:p3)}
...
hamilton_roles.each{|hr|CastMember.create(name:Faker::Name.name, role:hr, production:p4)}
